...
OlympiaSim::OlympiaSim(sparta::Scheduler & scheduler, const uint32_t num_cores,
                       const std::string workload, const uint64_t instruction_limit,
...
void OlympiaSim::buildTree_()
{
    ...
    sparta::ResourceTreeNode* cpu_tn =
        new sparta::ResourceTreeNode(getRoot(), "cpu", sparta::TreeNode::GROUP_NAME_NONE,
                                     sparta::TreeNode::GROUP_IDX_NONE, "CPU Node", cpu_factory);
    ...
    cpu_tn->addExtensionFactory("simulation_configuration",
                                [=]() { return new olympia::SimulationConfiguration; });
...
const sparta::CounterBase* OlympiaSim::findSemanticCounter_(CounterSemantic sem) const
{
    ...
    case CSEM_INSTRUCTIONS:
        return getRoot()->getChildAs<const sparta::CounterBase>(
            "cpu.core0.rob.stats.total_number_retired");
        break;
    default:
        return nullptr;
    }
